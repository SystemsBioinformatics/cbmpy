...
class Fbase(object):
    ...
    def __checkId__(self, cid=None):
        ...
        ----------
        cid
            [default=None] an optional Id to test for validity, if None then the object id is used
        """
        if cid is None:
            cid = self.id
    ...
    def serialize(self, protocol=0):
        ...
        ----------
        protocol
            [default=0] serialize to a string or binary if required,
        """
    ...
    def serializeToDisk(self, filename, protocol=2):
        ...
        protocol
            [default=2] serialize to a string or binary if required,
        """
        F = open(filename, 'wb')
    ...
    def addMIRIAMannotation(self, qual, entity, mid):
        ...
        mid
            the entity id e.g. CHEBI:17158 or fully qualifies url (if only_qual_uri)
        """
        if self.miriam == None:
    ...
    def addMIRIAMuri(self, qual, uri):
        ...
        qual
            a Biomodels biological qualifier e.g. "is" "isEncodedBy"
        uri
            the fully qualified entity id e.g. http://identifiers.org/chebi/CHEBI:12345 (no validity checking is done)
        """
        if self.miriam == None:
            self.miriam = MIRIAMannotation()
    ...
    def setSBOterm(self, sbo):
        ...
        ----------
        sbo
            the SBOterm with format: SBO:nnnnnnn"
        """
        assert (
...
class Model(Fbase):
    ...
    def hasObject(self, pid):
        ...
        ----------
        pid
            returns a boolean if there is a registered object with pid-
        """
    ...
    def createReaction(
        ...
        create_default_bounds
            create default reaction bounds, irreversible 0 <= J <= INF, reversable -INF <= J <= INF
        silent
    ...
    def createUserDefinedConstraint(self, pid, lb, ub, components=None):
        ...
        componentents
            optional, the user defined constraint componenents in the form of a list
        """
    ...
    def addUserConstraint(self, pid, fluxes=None, operator='>=', rhs=0.0):
        ...
        ----------
        pid
            user constraint name/id, use `None` for auto-assign
        fluxes
            a list of (coefficient, reaction id) pairs where coefficient is a float
        operator
            is one of '=', '>=' or '<=' (< and > will be interpreted as >= or <=)
        rhs
            a float
        """
...
#             R.setLowerBound(bound)
    ...
    def setAllFluxBounds(self, bounds):
        ...
        ----------
        bounds
            a dictionary of [fluxbound_id : value] pairs (not per reaction!!!)
        """
        print(
    ...
    def setObjectiveFlux(
        ...
        osense
            the optimization sense must be **maximize** or **minimize**
        delete_objflx
    ...
    def buildStoichMatrix(self, matrix_type='numpy', only_return=False):
        ...
        sympy
            a SymPy symbolic matrix, if available note the denominator limit can be set in ``CBModel.__CBCONFIG__['SYMPY_DENOM_LIMIT'] = 10**12``
        scipy_csr
            create using NumPy but store as SciPy csr_sparse
        only_return
            [default=False] **IMPORTANT** only returns the stoichiometric matrix and constraint matrix (if required),
        """
        if __DEBUG__:
...
class UserDefinedConstraint(Fbase):
    ...
    def getConstraintComponent(self, cid):
        ...
        ----------
        cid
            get a constraint component that matches cid
        """
...
class Compartment(Fbase):
    ...
    def __init__(self, pid, name=None, size=1, dimensions=3, volume=None):
        ...
        size
            compartment size, [default=1]
        dimensions
            compartment spatial dimensions [default=3]
        """
        pid = str(pid)
        if pid == self.id:
            return
    ...
    def setDimensions(self, dimensions):
        ...
        ----------
        dimensions
            set the new compartment dimensions
        """
        self.dimensions = dimensions
...
class Group(Fbase):
    ...
    def setSharedSBOterm(self, sbo):
        ...
        ----------
        sbo
            the SBOterm with format: "SBO:<7 digit integer>"
        """
...
class FluxBoundBase(Fbase):
    ...
    def __init__(self, pid, operator, value, parent=None):
        ...
        value
            a float
        parent
            [default=None] the parent reaction object
        """
        pid = str(pid)
        self.setId(pid)
...
class Parameter(Fbase):
    ...
    def __init__(self, pid, value, name=None, constant=True):
        ...
        ----------
        pid
            the unique parameter pid
        value
            the value
        name
            [default=''] the parameter name
        constant
            [default=True] is the paramter constant or can it be changed by a simulation/solver
        """
        pid = str(pid)
        self.setId(pid)
        if name is None:
...
class Reaction(Fbase):
    ...
    def getFVAdata(self, roundnum=None, silent=True):
        """
        Returns the data generated by CBSolver.FluxVariabilityAnalysis() for this reaction as a tuple of (Flux, FVAmin, FVAmax, span) where span is abs(FVAmax - FVAmin). FVAmin or FVAmax is None this indicates no solution to that particular optimization (infeasible).
    ...
    def getStoichiometry(self, use_names=False, altout=False):
        ...
        altout
            [default = False] returns a dictionary [DEPRECATED]
        """
        if not use_names:
    ...
    def getEquation(self, reverse_symb='=', irreverse_symb='>', use_names=False):
        ...
        irreverse_symb
            [default = '>'] the symbol to use for irreversible reactions
        use_names
            [defualt = False] use species names rather than id's
        """
        sub = ''
...
class Reagent(Fbase):
    ...
    def __init__(self, pid, species_ref, coef):
        ...
        pid
            a unique id
        species_ref
            a reference to a species id
        coefficient
            the stoichiometric coefficient, a non-zero integer or Parameter
        """
        self.setId(pid)
...
class Gene(Fbase):
    ...
    def __init__(self, pid, label=None, active=True):
        ...
        pid
            the gene id
        label
            the gene label this may or may not be a legal Sid
        active
            is the gene is active or not (boolean)
        """
        pid = str(pid)
        if not self.__checkId__(pid):
            raise RuntimeError(
...
class GeneProteinAssociation(Fbase):
    ...
    def __init__(self, pid, protein, use_compiled=False):
        ...
        pid
            a unique id
        protein
            the protein the gene association referes to, in most cases this should be a reaction id
        use_compiled
            [default=False] used compiled expressions for evaluation, potentially less portable
        """
        self.setPid(pid)
        self.generefs = []
    ...
    def createAssociationAndGeneRefsFromTree(self, gprtree, altlabels=None):
        ...
        altlabels
            [default=None] a dictionary containing a label<-->id mapping
        """
        if self.__objref__() == None:
            raise RuntimeError(
    ...
    def setTree(self, tree):
        ...
        ----------
        tree
            a dictionary representation of a GPR.
        """
        self.tree = tree
    ...
    def __getAssociationEvalFromGprDict__(self, gprd, out, parent=''):
        ...
        parent
            [default=''] the string representing the current nodes parent relationship, used for recursion
        model
            an FBA model with gene information
        """
