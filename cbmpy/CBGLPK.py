...
Last edit: $Author: bgoli $ ($Id: CBGLPK.py 713 2020-06-14 22:28:43Z bgoli $)
...
try:
    ...
    HAVE_SWIGLPK = True
    print('\n\nNot all methods implimented yet!')
except:
    raise ImportError


# configuration options for GLPK
GLPK_CFG = {'simplex': {'meth': sw.GLP_PRIMAL,
                        # 'meth' : sw.GLP_DUAL
                        # 'meth' : sw.GLP_DUALP
                        'tol_bnd': 1.0e-6,
                        'tol_dj': 1.0e-6,
...
def glpk_constructLPfromFBA(fba, fname=None):
    ...
    for r_ in range(Nmat.shape[0]):
        ...
        sw.glp_set_mat_row(lp, 1 + r_, Nmat.shape[1], sw.as_intArray(ind_arr), sw.as_doubleArray(val_arr))
