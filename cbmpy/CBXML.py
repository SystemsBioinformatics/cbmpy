...
def xml_stripTags(html):
    ...
    Returns
    -------
        a string with all HTML/XML tags removed.
    """
...
def formatSbmlId(s):
    """
    Format a string to be a valid SBML identifier. Convert a string to a valid SBML identifier by replacing non-alphanumeric characters with underscores and prefixing with underscore if it doesn't start with a letter or underscore.
...
def xml_addSBML2FBAFluxBound(document, rid, operator, value, fbid=None):
    ...
    ----------
    document
        a minidom XML document created by xml_createSBML2FBADoc
    rid
        the reaction id
    operator
        one of ['greater','greaterEqual','less','lessEqual','equal','>','>=','<','<=','=']
    value
...
def xml_createListOfFluxObjectives(document, l):
    ...
    document
        a minidom XML document created by xml_createSBML2FBADoc
    l
        a list of (reaction_id, coefficient) tuples to add as flux objectives
    
    Returns
    -------
        the listOfFluxes element.
    """
...
def xml_createSBML2FBAObjective(document, oid, sense, fluxObjectives):
    ...
    oid
        the objective ID
    sense
        a string containing the objective sense ('maximize' or 'minimize')
    fluxObjectives
        a list of (reaction_id, coefficient) tuples
...
def xml_addSBML2FBAObjective(document, objective, active=True):
    ...
    document
        a minidom XML document created by `xml_createSBML2FBADoc`
    objective
        a minidom XML objective element created with `xml_createSBML2FBAObjective`
    active
...
def xml_getSBML2FBAannotation(fba, fname=None):
    ...
    fba
        an fba model object
    fname
        [default=None] if supplied the XML will be written to file *fname*
    """
...
def sbml_setDescription(model, fba):
    """
    Set the model description as an HTML paragraph in a <note> element. Sets the model description as a <note> containing HTML in a paragraph element on the model object.
...
def sbml_setNotes3(obj, s):
    ...
    Returns
    -------
        True if notes were set successfully, False otherwise.
    """
...
def sbml_getNotes(obj):
    ...
    Returns
    -------
        the notes string, or an empty string if no notes exist.
    """
    notes = ''
    try:
        if libsbml.getLibSBMLVersion() >= 51903:
...
def sbml_setUnits(model, units=None, give_default=False, L3=True):
    ...
    model
        a libSBML model instance
    units
        [default=None] a dictionary of units (if None, default units are used)
    give_default
        [default=False] if True, returns the default unit dictionary
    L3
        [default=True] use SBML Level 3 defaults
    
    Returns
    -------
        the units dictionary if give_default is True, otherwise None.
    """
...
def sbml_writeAnnotationsAsCOBRANote(annotations):
    ...
    ----------
    annotations
        a dictionary of annotation keys to values
...
def sbml_setSpeciesL2(model, fba, return_dicts=False):
    ...
    ----------
    model
        a libSBML model instance (or None if return_dicts=True)
    fba
        a PySCeSCBM model instance
    return_dicts
        [default=False] if True, only return compartments and species dicts
    
    Returns
    -------
        the compartments dictionary (or (compartments, species) tuple if return_dicts=True).
    """
...
def sbml_setReactionsL2(model, fba, return_dict=False):
    ...
    fba
        a PySCeSCBM model instance
    return_dict
        [default=False] if True, return reaction dictionaries without adding to SBML
    
    Returns
    -------
        reaction dictionaries if return_dict is True, otherwise None.
    """
...
def sbml_exportSBML2FBAModel(
    ...
    filename
        the output filename
    directory
        [default=None] by default use filename otherwise join, <dir><filename>
    return_doc
...
class FBCconnect(object):
    ...
    def __init__(self, fbc_version=2, fbc_strict=True, enable_groups=False):
        ...
        ----------
        fbc_version
            [default=2] fbc version
        fbc_strict
            [default=True] if using FBC V2 set the strict flag
        enable_groups
            [default=False] use the Groups package if possible
        """
    ...
    def createGroupLegacy(self, gid, group, name=None):
        ...
        gid
            the group name
        groups
            a list of strings that refer to model SId's
        """
    ...
    def createGeneAssociationV1(self, rid, assoc, gprid=None):
        ...
        rid
            a reaction id
        assoc
            the association string e.g.(b0698 and b0697) or (b0696)
        """
...
class CBMtoSBML3(FBCconnect):
    ...
    def addBoundsV2(self, compress_bounds=False, sig_dig=20):
        ...
        ----------
        compress_bounds
            [default=False] enable parameter compression
        sig_dig
            [default=20] round off to significant digits
        """
        if compress_bounds:
...
# TODO Implementr
    ...
    def addGenesV2(
        ...
        parse_from_annotation
            [default=False] if the gpr_assoc list is empty e.g. the GPR associations have
        annotation_key
            [default='GENE ASSOCIATION'] the key to use for the gene association if parse_from_annotation is True
        add_cbmpy_anno
            [default=True] add PySCeS CBMPy annotation
        """
    ...
    def addGeneProteinAssociationsV1(
        ...
        annotation_key
            [default='GENE ASSOCIATION'] the key to use for the gene association if parse_from_annotation is True
        add_cbmpy_anno
            add PySCeS CBMPy annotation (still to be implemented)
        """
        if len(self.fba.gpr) == 0 and parse_from_annotation:
            print(
    ...
    def createParParameter(self, param, add_cbmpy_anno=True):
        ...
        ----------
        param
            a CBMPy parameter object
        add_cbmpy_anno
            [default=True] add annotation to SBML object
        """
...
def sbml_setSpeciesL3(
    ...
    ----------
    model
        an SBML model instance (or None if return_dicts=True)
    fba
        a PySCeSCBM model instance
    return_dicts
        [default=False] if True, only return compartments and species dicts
    ...
    substance_units
        [default=True] use amounts instead of concentrations
    fbc_version
...
def sbml_writeCOBRASBML(fba, fname, directory=None):
    """
    Write an FBA model as COBRA-compatible SBML. Takes an FBA model object and writes it to a file as COBRA-compatible SBML by first writing as SBML L3 FBC, then converting to COBRA format.
    ...
    fname
        the output filename
    directory
        [default=None] if provided, prepended to fname
...
def sbml_getGPRasDictFBCv1(node, out):
    ...
    ----------
    node
        a Python AST note (e.g. `ast.parse(gprstring).body[0]`)
    out
...
def sbml_convertCOBRASBMLtoFBC(fname, outname=None, work_dir=None, output_dir=None):
    ...
    fname
        is the filename
    outname
        the name of the output file. If not specified then <filename>.l3fbc.xml is used as default
    work_dir
        [default=None] is the working directory
    output_dir
        [default=None] is the output directory (default is work_dir)
    """
    assert (
        _HAVE_SBML_
...
def sbml_convertSBML3FBCToCOBRA(fname, outname=None, work_dir=None, output_dir=None):
    ...
    fname
        is the filename
    outname
        the name of the output file. If not specified then <filename>.cobra.xml is used as default
    work_dir
        [default=None] is the working directory
    output_dir
        [default=None] is the output directory (default is work_dir)
