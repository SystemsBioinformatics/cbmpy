...
def createSink(modelFile, to_sink, commonFileName, fileName, originalName):
    ...
    for byproduct in to_sink:
        if byproduct in sinkM:
            ...
            try:
                ...
                # Make model for individual sinks
                ...
                # Make a common sink model
                modelFileCommon.createReaction(sinkid, name = sinkname, reversible=True, create_default_bounds=True)
                modelFileCommon.createReactionReagent(sinkid, byproduct, -1)
                modelFileCommon.setReactionBounds(sinkid,0,999999)
                modelFileCommon.getReaction(sinkid).setAnnotation('SUBSYSTEM','Exchange reactions')
                modelFileCommon.getReaction(sinkid).setAnnotation('NAME',sinkname)
            
            except:
                print 'No sink reaction created for: '+byproduct

    return modelFileCommon, sinkIDs, sinkM
...
def findProduct(modelFile, biomass_reaction):
    ...
    commonFileName = fileName + '_allSinks.xml'
    # put orignal model file in common directory for later use
    ...
    modelFileCommon, sinkIDs, sinkM = createSink(modelFile,list(set(byproducts)), commonFileName, fileName, originalName)

    cbm.CBWrite.writeSBML3FBC(modelFileCommon, commonFileName, directory='Models/SynthSinks')

    return sinkIDs, byproducts
