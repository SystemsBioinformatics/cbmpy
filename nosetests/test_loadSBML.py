...
class TestReadSBML(object):
    ...
    def test_load_L3FBCV2_iJR904(self):
        cmod = cbmpy.readSBML3FBC(os.path.join(MDIR, 'L3FBCV2_iJR904.glc.xml'), xoptions={'validate' : True})
        assert_not_equal(cmod, None)

    def test_load_L3FBCV1_iJR904(self):
        cmod = cbmpy.readSBML3FBC(os.path.join(MDIR, 'L3FBCV1_iJR904.glc.xml'), xoptions={'validate' : True})
        assert_not_equal(cmod, None)
...
class TestRunFBA(object):
    ...
    def setup_class(klass):
        ...
        for m in DATA:
            ...
            if m.startswith('L2CBR_'):
                ...
                cmod = cbmpy.readSBML3FBC(os.path.join(MDIR, m), xoptions={'validate' : True})
            if cmod is not None:
                klass.m[m] = cmod
        klass.CDIR = cDir
    ...
    def test_run_fba_cobra(self):
        ...
        cmod = cbmpy.readCOBRASBML('L2CBR_iJR904.glc.xml', work_dir=MDIR,\
                                                        output_dir=self.CDIR, delete_intermediate=True)
        cmod2 = cbmpy.readSBML3FBC('L3FBCV1_iJR904.glc.xml', work_dir=MDIR, xoptions={'validate' : True})
